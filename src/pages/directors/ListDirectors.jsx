...
export const ListDirectors = () => {
  ...
  const getDirectors = async () => {
    try {
      const response = await axios.get(
        'https://proyecto--backend.herokuapp.com/api/v1/director',
  ...
  const deleteDirector = async (id) => {
    try {
      ...
      if (confirmation) {
        await axios.get(
          `https://proyecto--backend.herokuapp.com/v1/director/${id}/destroy`,
          { headers: { 'accept': 'application/json', 'authorization': token } }
        );
        await getDirectors();
      }
  ...
  return (
    <div>
      ...
      <hr className='mt-3' />
      <p className='mt-3'>Lista de directores creados</p>
              ...
              <tr key={director.id} className="border-b hover:bg-gray-100">
                ...
                <td className='p-3'>
                  <button type='button' className='bg-sky-800 block w-full text-white p-2 uppercase font-bold text-xs rounded-xl'
                    onClick={() => navigate(`/directors/show/${director.id}`)}>Ver</button>
